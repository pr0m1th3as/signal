...
#include <stdio.h>
  ...
  #define acosh(x) log((x) + sqrt(1. * (x) * (x) - 1))
#endif
...
static double * ultraspherical_win(int n, double mu, double xmu)
{
  double * w = NULL;
...
static uspv_t ultraspherical_polyval(
...
static uspv_t ultraspherical_polyval2(     /* With non-+ve integer protection */
    int n, double mu, double x, double const * divs)
{
...
static double find_zero(int n, double mu, int l, double extremum_mag,
    double ripple_ratio, double lower_bound, double const * divs)
{
...
static double * make_divs(int n, double * * divs)
{
  int i;
  if (!*divs) {
...
double * ultraspherical_window(
    int n, double mu, double par, uswpt_t type, int even_norm, double * xmu_)
{
